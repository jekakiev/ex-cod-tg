...
from bot.launchd import (
    install_helper_launch_agent,
    install_launch_agent,
    restart_helper_launch_agent,
    restart_launch_agent,
    uninstall_helper_launch_agent,
    uninstall_launch_agent,
)
from bot.main import run_bot
from bot.systemd import install_systemd_service, restart_systemd_service, uninstall_systemd_service
from bot.tray import main as tray_main
...
def build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser("tray", help="Start the macOS menu bar helper.")
...
def service_command(paths: AppPaths, action: str) -> int:
    if action == "install":
        ...
        service_paths = install_service(paths, executable)
        print(f"Installed {paths.service_kind} service: {service_paths[0]}")
        if len(service_paths) > 1:
            print(f"Installed helper: {service_paths[1]}")
        print(f"Logs: {paths.log_file}")
        if paths.helper_log_file:
            print(f"Helper logs: {paths.helper_log_file}")
        return 0
    if action == "uninstall":
        removed = uninstall_service(paths)
        if removed:
            print(f"Removed {paths.service_kind} service: {paths.service_file}")
            if paths.helper_service_file:
                print(f"Removed helper service: {paths.helper_service_file}")
        else:
    ...
    if action == "restart":
        ...
        if paths.helper_service_label:
            print(f"Restarted helper service: {paths.helper_service_label}")
        print(f"Logs: {paths.log_file}")
        return 0
    raise RuntimeError(f"Unknown service action: {action}")


def install_service(paths: AppPaths, executable: Path) -> list[Path]:
    if paths.service_kind == "launchd":
        installed = [install_launch_agent(paths, executable)]
        if paths.helper_service_file and paths.helper_service_label:
            installed.append(install_helper_launch_agent(paths, executable))
        return installed
...
def uninstall_service(paths: AppPaths) -> bool:
    if paths.service_kind == "launchd":
        removed_helper = uninstall_helper_launch_agent(paths) if paths.helper_service_file else False
        removed_bot = uninstall_launch_agent(paths)
        return removed_bot or removed_helper
    if paths.service_kind == "systemd":
...
def restart_service(paths: AppPaths) -> None:
    if paths.service_kind == "launchd":
        ...
        if paths.helper_service_file and paths.helper_service_label:
            restart_helper_launch_agent(paths)
        return
    if paths.service_kind == "systemd":
